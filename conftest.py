...
def postgres_container() -> PostgresContainer:
    ...
    container.get_container_host_ip = localhost
    container.start()
    return container
...
def postgres_url(postgres_container: PostgresContainer) -> str:
    engine = create_engine(postgres_container.get_connection_url().replace("localnpipe", "localhost"))
    conn = engine.connect()
...
def conn_with_data(postgres_container: PostgresContainer) -> str:
    ...
    return postgres_container.get_connection_url().replace("localnpipe", "localhost")
